...
describe("POST /api/elaborate", () => {
  ...
  it("handles POST request and returns a JSON response", async () => {
    ...
    const req = {
      ...
      url: `${NEXT_PUBLIC_BASE_URL}/api/elaborate`,
    ...
    expect(fetchSpy).toHaveBeenCalledWith(`${NEXT_PUBLIC_BASE_URL}/elaborate`, {
      method: "POST",
...
describe("POST /api/elaborate - Error Handling", () => {
  let fetchSpy: jest.SpyInstance;
  let jsonMock: jest.SpyInstance;
...
describe("POST /api/elaborate - Error Handling for !response.ok", () => {
  let fetchSpy: jest.SpyInstance;
  let jsonMock: jest.SpyInstance;
