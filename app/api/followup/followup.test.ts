...
describe("POST /api/followup", () => {
  ...
  it("handles POST request and returns a JSON response", async () => {
    ...
    const req = {
      ...
      url: `${NEXT_PUBLIC_BASE_URL}/api/followup`,
    ...
    expect(fetchSpy).toHaveBeenCalledWith(`${NEXT_PUBLIC_BASE_URL}/followup`, {
      method: "POST",
...
describe("POST /api/followup - Error Handling", () => {
  let fetchSpy: jest.SpyInstance;
  let jsonMock: jest.SpyInstance;
...
describe("POST /api/followup - Error Handling for !response.ok", () => {
  let fetchSpy: jest.SpyInstance;
  let jsonMock: jest.SpyInstance;
